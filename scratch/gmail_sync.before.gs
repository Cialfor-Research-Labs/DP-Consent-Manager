...
// ─────────────────────────────────────────────────────────────
// SYSTEM-GENERATED MESSAGE FILTER (DEFENSE IN DEPTH)
// ─────────────────────────────────────────────────────────────

function isSystemGeneratedEmail(fromRaw, subject, bodyText, fiduciaryName) {

  var fromLower = String(fromRaw || '').toLowerCase();
  var subStr = String(subject || '').trim();
  var bodyStr = String(bodyText || '');
  var fidLower = String(fiduciaryName || '').toLowerCase();

  // 1. Sender / Fiduciary matches portal identity
  if (
    fidLower.indexOf('dpdp privacy portal') > -1 ||
    fromLower.indexOf('dpdp privacy portal') > -1
  ) {
    return { isSystem: true, reason: 'sender_is_dpdp_privacy_portal' };
  }

  // 2. Specific portal system signatures
  var portalSignatures = [
  ...
  for (var i = 0; i < portalSignatures.length; i++) {
    if (bodyStr.indexOf(portalSignatures[i]) > -1) {
      return {
        isSystem: true,
        reason: 'portal_signature_' + portalSignatures[i].replace(/\s+/g, '_').toLowerCase()
      };
    }
  }

  // 3. Subject starts with "Re:" AND contains consent status/decision indicators
  if (/^re:\s*/i.test(subStr)) {
    var replyIndicators = [
      'CONSENT GRANTED',
      'CONSENT DENIED',
    ...
    for (var j = 0; j < replyIndicators.length; j++) {
      if (
        bodyStr.indexOf(replyIndicators[j]) > -1 ||
        subStr.indexOf(replyIndicators[j]) > -1
      ) {
        return {
          isSystem: true,
          reason: 'reply_marker_' + replyIndicators[j].replace(/\s+/g, '_').toLowerCase()
        };
      }
    }
  }

  // 4. Automated DP statutory notices generated by our sync script
  if (
    /^new consent request:/i.test(subStr) &&
    bodyStr.indexOf('automated statutory notice under the Digital Personal Data Protection') > -1
  ) {
    return { isSystem: true, reason: 'automated_statutory_notice' };
  }
          ...
          thread
            ...
            .forEach(
              function(msg) {
                ...
                // ── SYSTEM-GENERATED MESSAGE CHECK ────────
                var systemCheck =
                ...
                if (systemCheck.isSystem) {
                  ...
                  Logger.log(
                    '[SKIP-SYSTEM] Ignored self-generated message: "' +
                    subject +
                    '" (' +
                    systemCheck.reason +
                    ')'
                    ...
                    try {
                      ...
                      if (parsed.ignored) {
                        ...
                        Logger.log(
                          ...
                          subject +
                          '" (' +
                          (parsed.reason || 'system_generated') +
                          ')'
                        );
                      ...
                      // ── PART 5: DATA PRINCIPAL EMAIL NOTIFICATION ──
                      var dpRecipient = (parsed.dataPrincipal && parsed.dataPrincipal.email) ? parsed.dataPrincipal.email : effectiveTo;
                      var isNewRequest = (parsed.is_new !== false);
                      if (isNewRequest && !props.getProperty('dp_notified_' + msgId) && isValidEmail(dpRecipient)) {
                        try {
                          var notifSubject = 'New Consent Request: ' + fiduciaryName;
                          var notifBody = 
                            'New Consent Request\n\n' +
                            'Fiduciary:\n' + fiduciaryName + '\n\n' +
                            'Purpose:\n' + (parsed.purpose || subject) + '\n\n' +
                            'Notice ID:\n' + (parsed.noticeId || parsed.notice_id || 'NTC-GEN') + '\n\n' +
                            'Review Consent Request:\n' + consentLink + '\n\n' +
                            'This is an automated statutory notice under the Digital Personal Data Protection (DPDP) Act 2023.';
                          
                          GmailApp.sendEmail(dpRecipient, notifSubject, notifBody);
                          props.setProperty('dp_notified_' + msgId, new Date().toISOString());
                          Logger.log('         → Notified Data Principal: ' + dpRecipient);
                        } catch (notifErr) {
                          Logger.log('         [WARN] DP notification failed: ' + notifErr.toString());
                        }
                      }
...
function setupTrigger() {
  ...
  Logger.log(
    'syncConsentEmails() will run every ' + interval + ' minute(s).'
  );
...
function dispatchPendingConsentReplies() {
  ...
  try {
    ...
    Logger.log(
      '=== Dispatch Complete (Dispatched: ' + dispatchedCount + ', Acked: ' + ackedCount + ', Failed: ' + failedCount + ') ==='
    );
...
// ─────────────────────────────────────────────────────────────
// WEBHOOK / EVENT-DRIVEN DISPATCH HANDLERS
// ─────────────────────────────────────────────────────────────
//
// Deploy this script as a Web App in Google Apps Script:
// Deploy > New deployment > Select type: Web App
// - Description: Immediate Consent Receipt Dispatcher
// - Execute as: Me (<your-email>)
// - Who has access: Anyone
//
// Copy the Web App URL (https://script.google.com/macros/s/.../exec)
// and set it in your backend environment as:
// APPS_SCRIPT_WEBAPP_URL=https://script.google.com/macros/s/.../exec
//
// When a consent decision is recorded, FastAPI immediately POSTs to this
// URL to trigger dispatchPendingConsentReplies() within milliseconds.
// ─────────────────────────────────────────────────────────────

function doPost(e) {
  return handleDispatchWebhook(e);
}
...
function handleDispatchWebhook(e) {
  try {
    var expectedSecret = getWebhookSecret();
    var providedSecret = null;

    if (e && e.parameter && e.parameter.secret) {
      providedSecret = e.parameter.secret;
    } else if (e && e.parameter && e.parameter['X-Webhook-Secret']) {
      providedSecret = e.parameter['X-Webhook-Secret'];
    }

    if (!providedSecret && e && e.postData && e.postData.contents) {
      try {
        var parsed = JSON.parse(e.postData.contents);
        providedSecret = parsed.secret || parsed['X-Webhook-Secret'] || parsed.webhook_secret;
      } catch (jsonErr) {
    ...
    if (!providedSecret || providedSecret.trim() !== expectedSecret.trim()) {
      Logger.log('[DISPATCH-WEBHOOK] Unauthorized access attempt.');
    ...
    Logger.log('[DISPATCH-WEBHOOK] Authenticated immediate dispatch request received.');
    var dispatchResult = dispatchPendingConsentReplies();
    ...
    Logger.log('[DISPATCH-WEBHOOK-ERROR] ' + err.toString());
    return ContentService.createTextOutput(JSON.stringify({
      status: 'error',
      code: 500,
